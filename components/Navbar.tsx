  ...
  return (
    <header
      className={cn(
        ...
        scrolled && lightMode
          ? "bg-bone/88 py-3 text-obsidian shadow-[0_12px_48px_rgba(5,5,5,.12)] backdrop-blur-2xl"
          : scrolled
            ? "bg-obsidian/70 py-3 text-bone shadow-glass backdrop-blur-2xl"
            : "bg-transparent py-5 text-bone"
      )}
    >
      <nav className="container-premium flex items-center justify-between gap-5">
        <a
          href="#inicio"
        ...
        >
          <span className="relative grid size-14 shrink-0 place-items-center rounded-full border border-sand/45 bg-[#07111d] shadow-[0_14px_40px_rgba(0,0,0,.36)] transition group-hover:scale-105 group-hover:border-sand">
            <Image
          ...
          <span className="min-w-0">
            <span className="block truncate font-display text-2xl font-semibold leading-none tracking-normal 2xl:text-3xl">
              Kuntur Travel
            </span>
            <span
              className={cn(
                "mt-1 hidden text-[0.62rem] font-extrabold uppercase tracking-[0.28em] sm:block",
        ...
        <div
          className={cn(
            "hidden items-center gap-1 rounded-full border px-2 py-2 shadow-[inset_0_1px_0_rgba(255,255,255,.08)] backdrop-blur-2xl lg:flex",
            lightMode && scrolled
        ...
        >
          {links.map((link) => (
            <a
              ...
              href={link.href}
              className={cn(
                "rounded-full px-3 py-2 text-sm font-extrabold transition duration-300 2xl:px-4",
                lightMode && scrolled
